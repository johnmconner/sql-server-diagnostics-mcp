/*
Provision a Windows-authenticated read-only login for SQL TShooter.
...
DECLARE @WindowsPrincipal sysname = N'DOMAIN\sql_tshooter_ro';
...
IF SUSER_ID(@WindowsPrincipal) IS NULL
BEGIN
    DECLARE @CreateLoginSql nvarchar(max) =
        N'CREATE LOGIN ' + QUOTENAME(@WindowsPrincipal) + N' FROM WINDOWS;';
    EXEC(@CreateLoginSql);
END;

DECLARE @CreateUserSql nvarchar(max) =
    N'USE ' + QUOTENAME(@TargetDatabase) + N';
      IF USER_ID(' + QUOTENAME(@WindowsPrincipal, '''') + N') IS NULL
      BEGIN
          CREATE USER ' + QUOTENAME(@WindowsPrincipal) + N' FOR LOGIN ' + QUOTENAME(@WindowsPrincipal) + N';
      END;';
EXEC(@CreateUserSql);
...
DECLARE @GrantServerPermissionSql nvarchar(max) =
    N'USE [master];
      IF NOT EXISTS (
          ...
          WHERE sp.name = ' + QUOTENAME(@WindowsPrincipal, '''') + N'
            AND p.permission_name = ' + QUOTENAME(@ServerPermission, '''') + N'
      )
      BEGIN
          GRANT ' + QUOTENAME(@ServerPermission) + N' TO ' + QUOTENAME(@WindowsPrincipal) + N';
      END;';
        ...
        N'USE [msdb];
          ...
          AND NOT EXISTS (
              ...
              WHERE role_principal.name = N''SQLAgentReaderRole''
                AND member_principal.name = ' + QUOTENAME(@WindowsPrincipal, '''') + N'
          )
          BEGIN
              ALTER ROLE [SQLAgentReaderRole] ADD MEMBER ' + QUOTENAME(@WindowsPrincipal) + N';
          END;';
    EXEC(@GrantMsdbSql);
END;

SELECT
    @WindowsPrincipal AS login_name,
